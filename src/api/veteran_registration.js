...
export const registerVeteran = async ({
  ...
  try {
    ...
    const response = await fetch(veteranRegistrationURL, {
    ...
    if (response.ok) {
      ...
      console.log('Veterans Registration submitted.')
      console.log(result)
    } else if (response.status === 500) {
      console.log(
        'Veterans Registration already submitted once, please wait until the verification is complete.'
      )
    } else {
